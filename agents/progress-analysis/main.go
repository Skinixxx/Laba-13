...
func main() {
	...
	_, err = nc.Subscribe("tasks.progress.analyze", func(m *nats.Msg) {
		...
		var req shared.ProgressRequest
		...
		span.SetAttributes(
			...
			attribute.Int("activity.entries", len(req.ActivityLog)),
		)

		output := analyzeProgress(req)
		span.SetAttributes(
			attribute.Float64("progress.completion_pct", output.CompletionPct),
			attribute.Float64("progress.avg_score", output.AvgScore),
			attribute.String("progress.trend", output.Trend),
			attribute.Int("progress.weak_topics", len(output.WeakTopics)),
		)
